...
def test_try_delete_not_my_pet():
    """Проверяем возможность удалить чужого питомца"""
    ...
    if len(my_pets['pets']) > 0:
        ...
        # И первый питомец из общего списка это не мой питомец
        if all_pets['pets'][0]['id'] != my_pets['pets'][0]['id']:
            ...
            # Если первый питомец это мой питомец - вывести исключение о том, что это мой питомец
            raise Exception("First pet is my pet")

    # Если своих питомцев нет
    else:
        # Берём id первого питомца из списка всех питомцев и отправляем запрос на удаление
        pet_id = all_pets['pets'][0]['id']
        status, _ = pf.delete_pet(auth_key, pet_id)
...
def test_update_not_my_pet_info(name='тест', animal_type='тестовый', age='9'):
    """Проверяем возможность обновить информацию чужого питомца"""
    ...
    if len(my_pets['pets']) > 0:
        ...
        if all_pets['pets'][0]['id'] != my_pets['pets'][0]['id']:
            ...
            # Проверяем что статус ответа равен 200 и имя питомца изменилось
            assert status == 200
...
def test_add_new_pet_with_incorrect_data(name='', animal_type='', age='', pet_photo='images/45435.jpg'):
    """Проверяем возможность добавить питомца с неуказанным именем, породой и возрастом"""
    ...
    # Сверяем полученный ответ и имя питомца
    assert status == 200
    assert result['name'] == name
...
def test_add_new_pet_with_invalid_photo(name='Бар', animal_type='двортерьер',
    ...
    """Проверяем возможность добавить питомца с некорректным форматом фото"""
...
def test_update_self_pet_info_with_incorrect_data(name=big_name, animal_type=symbol_type, age='dededddd'):
    """Проверяем возможность обновить информацию о питомце некорректными данными"""
    ...
    if len(my_pets['pets']) > 0:
        ...
        # Проверяем статус ответа = 200 и что имя питомца соответствует заданному.
        assert status == 200
        assert result['name'] == name
    else:
...
def test_add_new_simple_pet_with_incorrcet_data(name=symbol_type, animal_type=big_name,
                                                age='sgsdggdsgdg'):
    """Проверяем возможность добавить простого питомца без фото с некорректными данными"""
...
def test_add_pet_photo_with_invalid_file(pet_photo='images/test_txt.txt'):
    ...
    if len(my_pets['pets']) > 0:
        ...
        # Проверяем что статус ответа = 500 и фото питомца не изменилось
        assert status == 500
